...
![wmiexec connection](../images/image25.png)
...
![Wazuh dashboard NTLM logon](../images/image34.png)
...
![Terminal commands](../images/image32.png)
...
```xml
<group name="sysmon,wmi,execution,">
  <rule id="100001" level="12">
    <if_group>sysmon_eid1_detections</if_group>
    <field name="win.eventdata.parentImage">WmiPrvSE</field>
    <description>WMI remote connection executed a command: $(win.eventdata.image) spawned by WmiPrvSE.exe</description>
    <mitre>
        <id>T1047</id>
    </mitre>
  </rule>
...
3. \<if_group\> is responsible for filtering out what the group would be. Sysmon logs process creations – like ones in cmd.exe – as event ID one, this gives our SIEM exactly what to look out for.
...
A thing to note is that this rule would generate false positives if a legitimate user were to login using WMI, creating a false positive. However the chances of that is extremely low enough to warrant this rule; even if there is a chance of a false positive. Now let's log back onto Kali and mess around with the Windows system.

![wmiexec session](../images/image29.png)

Checking back in with the Wazuh Dashboard and there we have it:

![Dashboard alerts](../images/image22.png)

![Rule 100001 alert details](../images/image18.png)
...
With attack 0 out of the way, our attacker has gained access to the system. Now with the beginning of attack 1, he will scope out the system he has. Follow along [here](../Attack%201:%20System%20Information%20Discovery/README.md).
